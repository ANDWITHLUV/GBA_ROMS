# POCKET AUDITION
Pocket Audition may allow users to quickly and simply build instruments using the GBA's four PSG channels and test their sounds wihtin a limited range. Instruments may be able to play sounds in octaves [2,8] using keys {C , C#, D , D#, E , F , F#, G, G#, A , A#, B}. The GbA festures four PSG channels: Duty+Sweep, Duty, Wave, and Noise. This application follows the same ordering, except the channels are offset by one. While the GbA produces frequencies from user provided rates, this application does not calculate them, instead it looks them up in a table. The formula for calculating rates can be found within various guides. The calculations this aplication uses in its LUT were ceiled before differing with 2048. The tunings for C2 are: RATE octave_2 {43,156,262,363,457,546,631,710,785,856,922,986}. Users may need to take note that the GBA's noise channel four seems to feature only 60 distinct states so their is an ovelap past the greatest state given that the user is provided 84 keys. Acording to the various sources available that document the GBA's noise chanel four. higher frequency will make the noise to appear 'softer' and Depending on the frequency the noise will appear 'harder' or 'softer'. The GBA's noise chanel four does not use rate calculations.  
...
Key bindings can be found in the settings area's right hand side. These key bindings will change as you employ them.
 The aplication features four main sections: Keys, Scales, Controls, and Settings. 
